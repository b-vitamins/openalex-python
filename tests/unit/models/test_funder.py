...
class TestFunderModel:
    ...
    def test_funder_date_parsing_edge_cases(self):
        """Test that funder date parsing handles edge cases correctly."""
        from openalex.models import Funder

        # Test overflow seconds handling
        funder_data = {
            "id": "https://openalex.org/F1",
            "display_name": "Test Funder",
            "updated_date": "2020-01-02T00:00:61",
        }
        funder = Funder(**funder_data)
        assert funder.updated_date == date(2020, 1, 2)

    def test_funder_country_code_validation(self):
        """Test that country code validation and normalization works correctly."""
        from openalex.models import Funder
        ...
        with pytest.raises(ValidationError):
            invalid_data = {
                "id": "https://openalex.org/F2",
                "display_name": "Invalid Funder",
                "country_code": "USA",
            }
            Funder(**invalid_data)

    def test_funder_government_detection(self):
        """Test that government funder detection works correctly."""
        from openalex.models import Funder

        gov_data = {
            "id": "https://openalex.org/F1",
            "display_name": "Federal Research Agency",
        }
        gov_funder = Funder(**gov_data)
        assert gov_funder.is_government_funder()

        private_data = {
            "id": "https://openalex.org/F2",
            "display_name": "Private Research Foundation",
        }
        private_funder = Funder(**private_data)
        assert not private_funder.is_government_funder()

    def test_funder_funding_per_work_calculation(self):
        """Test that funding per work calculation handles edge cases."""
        from openalex.models import Funder
        ...
        no_work_data = {
            "id": "https://openalex.org/F2",
            "display_name": "No Work Funder",
            "grants_count": 10,
            "works_count": 0,
        }
        no_work_funder = Funder(**no_work_data)
        assert no_work_funder.funding_per_work is None

    def test_funder_year_based_helpers(self):
        """Test that year-based helper methods work correctly."""
    ...
    def test_funder_summary_stats_access(self):
        """Test that summary stats properties are accessible."""
        from openalex.models import Funder, SummaryStats
