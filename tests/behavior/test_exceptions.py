...
class TestExceptionBehavior:
    ...
    def test_connection_operation_detection(self):
        ...
        def mock_request(method, url, **kwargs):
            ...
            return Mock(status_code=200, json=Mock(return_value={"results": []}), is_success=True)

        with patch.object(conn, "_client") as mock_client:
            mock_client.request = mock_request

            conn.request("GET", "https://api.openalex.org/autocomplete/works", {"q": "test"})
            assert timeout_used.read == 0.5

            conn.request("GET", "https://api.openalex.org/works/W123")
            assert timeout_used.read == 1.0

            conn.request("GET", "https://api.openalex.org/works", {"filter": {"search": "climate"}})
            assert timeout_used.read == 3.0

            conn.request("GET", "https://api.openalex.org/works", {"page": 1})
            assert timeout_used.read == 2.0
